...
function draftDeskView(){
 ...
 const order=remaining.map((n,i)=>{const t=pickerForPick(n),round=roundForPick(n),divider=i===0||round!==roundForPick(remaining[i-1])?`<div class="desk-round-divider" role="heading" aria-level="3">Round ${round}</div>`:'';return `${divider}<div class="desk-order-item ${n===s.current_pick_no&&s.status==='live'?'is-current':''} ${t?.id===me?.id?'is-mine':''}"><span class="desk-order-number">${n}</span><div><strong>${escapeHtml(t?.name||'—')}</strong><small>Round ${roundForPick(n)}</small></div></div>`}).join('');
...
function bindEvents(){
  ...
  app.querySelector('[data-action="join"]')?.addEventListener('click',join);app.querySelector('[data-action="logout"]')?.addEventListener('click',logout);app.querySelector('[data-action="toggle-sound"]')?.addEventListener('click',toggleSound);
