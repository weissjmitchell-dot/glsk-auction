...
function draftDeskView(){
 ...
 const order=upcoming.map((x,i)=>`${i===0||x.round!==upcoming[i-1].round?`<div class="desk-round-divider" role="heading" aria-level="3">Round ${x.round}</div>`:''}<div class="desk-order-item ${i===0&&s.status==='live'?'is-current':''} ${x.team.id===me?.id?'is-mine':''}"><span class="desk-order-number">${x.turn}</span><div><strong>${escapeHtml(x.team.name)}</strong><small>Round ${x.round}</small></div></div>`).join('');
